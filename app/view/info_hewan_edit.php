...
<body class="bg-gray-50 font-sans text-gray-800">
          ...
          <?php $no = 1; foreach ($data as $row): ?>
            <tr class="bg-white hover:bg-cyan-50 shadow-sm rounded">
              ...
              <td class="px-4 py-3 space-x-2">
                <a href="edit_hewan.php?id=<?= $row['id'] ?>" class="text-primary hover:underline">Edit</a>
  ...
  <div id="popupModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 hidden" role="dialog" aria-modal="true">
      ...
      <form action="../proses/proses_pembagian.php" method="POST" class="space-y-6">
        <div>
          ...
          <select name="jenis" required
            class="w-full rounded-xl border border-gray-300 px-4 py-3 focus:ring-2 focus:ring-primary bg-white text-gray-700">
        ...
        <div class="flex justify-end">
          <button type="submit"
            class="bg-primary hover:bg-primary-dark text-white font-semibold py-3 px-6 rounded-xl shadow-md transition text-sm">
            Bagikan Sekarang
          </button>
        </div>
      </form>
  ...
  <div id="successModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-3xl p-8 w-full max-w-md relative shadow-xl border border-green-500">
      <button onclick="toggleSuccessModal()" class="absolute top-4 right-5 text-gray-500 hover:text-red-600 text-xl font-bold">&times;</button>
      <h2 class="text-center text-green-600 font-bold text-xl mb-4">Sukses!</h2>
      <p class="text-center text-gray-700">Daging sukses dibagikan.</p>
    </div>
  </div>
